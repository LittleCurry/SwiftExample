...
- (void)setBackgroundImage:(UIImage *)backgroundImage {
    ...
    cardsMap[nullPosition.myX][nullPosition.myY] = nil;
}
...
- (void)moveCard:(YFCard *)card toPosition:(Position)position {
    ...
    cardsMap[card.position.myX][card.position.myY] = nil;
    nullPosition = card.position;

    cardsMap[position.myX][position.myY] = card;
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            ...
            if (cardsMap[i][j]) {
                ...
                if (card.originPosition.myX != card.position.myX ||
                    card.originPosition.myY != card.position.myY) {
                    success = NO;
                }
            }
        }
...
-(Direction)getDirectionWithCard:(YFCard *)touchedCard {
    ...
    int x = touchedCard.position.myX;  // 2
    int y = touchedCard.position.myY;  // 3   (y,x)--->(3,2)
...
- (void)randomBreak {
    ...
    for (int i = 0; i < 1000; i++) {
        ...
        int x = nullPosition.myX;
        int y = nullPosition.myY;
        switch (dir) {
            case DIR_LEFT:
                x--;
                break;
...
- (Direction)getValidRandomDirect {
    ...
    switch (dir) {
        case DIR_LEFT:
            if (nullPosition.myX-1 == -1) {
                return [self getValidRandomDirect];
            }
            break;
        case DIR_UP:
            if (nullPosition.myY-1 == -1) {
                return [self getValidRandomDirect];
            }
            break;
        case DIR_RIGHT:
            if (nullPosition.myX+1 == 4) {
                return [self getValidRandomDirect];
            }
            break;
        case DIR_DOWN:
            if (nullPosition.myY+1 == 4) {
                return [self getValidRandomDirect];
            }
            break;
