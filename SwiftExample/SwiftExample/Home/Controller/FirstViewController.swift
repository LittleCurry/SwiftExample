...
class FirstViewController: BaseViewController, UITableViewDelegate, UITableViewDataSource{
    // 这是写属性的地方
    var nameArr = ["navigationBar渐变", "聊天界面", "gauss模糊", "Share", "Map", "二维码", "视频播放", "block", "天气", "清除缓存", "日期选择", "我的银行卡", "本地相册选取", "轮播图", "热更新", "弹幕", "日历", "商城首页", "商城分类", "商城发现", "商城购物车", "商城我的", "请输入密码", "圆形菜单", "球形标签", "推荐影片", "渐隐文字", "打印文字", "拼图", "2048", "浏览卡片", "抽卡效果", "抖动菜单", "渐变色", "折卡效果", "卡牌拖动", "摇一摇", "ape展开", "余额跳动", "水平滚动布局", "发散菜单", "漂浮的雪花", "水波", "闹钟设置", "音乐播放", "某月的日期", "分开设置时分", "手势解锁", "拖拽排序", "侧边栏"];
...
//            self.navigationController?.pushViewController(menuVC, animated: true);
        ...
        case 42:
            ...
            break;
        case 43:
            let clockVC = FirstClockViewController.init();
            clockVC.hidesBottomBarWhenPushed = true;
            self.navigationController?.pushViewController(clockVC, animated: true);
            break;
        case 44:
            let recordVC = FirstTimeRecordViewController.init();
            recordVC.hidesBottomBarWhenPushed = true;
            self.navigationController?.pushViewController(recordVC, animated: true);
            break;
        case 45:
            let monthVC = FirstMonthCircleViewController.init();
        ...
        case 47:
            ...
            break;
        case 48:
            let dragVC = DraggingViewController.init();
            dragVC.navigationItem.title = "拖拽排序"
            dragVC.hidesBottomBarWhenPushed = true;
            self.navigationController?.pushViewController(dragVC, animated: true);
            break;
        case 49:
            let psDrawerVC = PSTabBarController.init();
            psDrawerVC.hidesBottomBarWhenPushed = true;
            let leftView = LeftView.init(frame: CGRect(x:-WIDTH*(1-0.75), y:0, width:WIDTH, height:HEIGHT))
            let tabBarVC = PSTabBarController.init()
            PSDrawerManager.instance().installCenter(tabBarVC, leftView: leftView)
